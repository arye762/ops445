...
class TestArgs(unittest.TestCase):
    ...
    def test_argparse_help(self):
        ...
        p = subprocess.Popen(['/usr/bin/python3', 'duim.py', '-h'], stdout=subprocess.PIPE, stdin=subprocess.PIPE,
...
class TestPercent(unittest.TestCase):
    ...
    def test_percent(self):
        ...
        for i in range(0, (len(percent_list) - 1)):
            ...
            expected = ' ' * num_spcs
            error_msg = "The output of percent_to_graph() with the argument " + str(
                percent_list[i]) + " is returning the wrong value"
...
class TestDirDict(unittest.TestCase):
    ...
    def test_dir_dict_func(self):
        ...
        test_dat = ['164028\t/usr/local/lib/heroku', '11072\t/usr/local/lib/python2.7',
                    '92608\t/usr/local/lib/node_modules', '8\t/usr/local/lib/python3.8', '267720\t/usr/local/lib']
        expected = {'/usr/local/lib/heroku': 164028, '/usr/local/lib/python2.7': 11072,
                    '/usr/local/lib/node_modules': 92608, '/usr/local/lib/python3.8': 8, '/usr/local/lib': 267720}
